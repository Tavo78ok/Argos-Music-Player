...
class AudioEngine:
    def __init__(self, on_eos, on_error, on_position):
        ...
        # EQ: usar audio-filter de playbin3 solo si el elemento existe
        # Se conecta una sola vez al inicio, no se reconecta durante reproducción
        self.eq=Gst.ElementFactory.make("equalizer-10bands","eq")
        self.eq_available=self.eq is not None
        if self.eq_available:
            try:
                # Construir filtro: capsfilter ! equalizer-10bands
                # Usar un bin para encapsularlo limpiamente
                fbin = Gst.Bin.new("eq-bin")
                conv = Gst.ElementFactory.make("audioconvert","eq-conv")
                fbin.add(self.eq)
                if conv:
                    fbin.add(conv)
                    conv.link(self.eq)
                    pad_in = Gst.GhostPad.new("sink", conv.get_static_pad("sink"))
                else:
                    pad_in = Gst.GhostPad.new("sink", self.eq.get_static_pad("sink"))
                pad_out = Gst.GhostPad.new("src", self.eq.get_static_pad("src"))
                fbin.add_pad(pad_in); fbin.add_pad(pad_out)
                self._players[0].set_property("audio-filter", fbin)
                self._eq_bin = fbin
            except Exception as e:
    ...
    def _xfade_tick(self):
        ...
        if self._xfade_step>=self._xfade_total:
            ...
            # El EQ solo está en el player 0; el crossfade
            # simplemente cambia volúmenes, sin reconectar el EQ
            return False
        return True
...
# ══ Utilidad: color dominante de portada ═══════════════════════════════
def dominant_color(pixbuf):
    """Devuelve (r,g,b) 0-255 del color más saturado/vibrante de la imagen."""
    try:
        # Escalar a miniatura para velocidad
        thumb = pixbuf.scale_simple(32, 32, GdkPixbuf.InterpType.BILINEAR)
        n_channels = thumb.get_n_channels()
        data = thumb.get_pixels()
        w, h = thumb.get_width(), thumb.get_height()
        rowstride = thumb.get_rowstride()

        best_score = -1
        best = (70, 130, 180)  # azul por defecto

        for y in range(h):
            for x in range(w):
                off = y * rowstride + x * n_channels
                r, g, b = data[off], data[off+1], data[off+2]
                # Convertir a HSL para buscar el más saturado y no muy oscuro/claro
                ...
                lightness_ok = 0.2 < l < 0.8
                score = s * (1 - abs(2*l - 0.55)) if lightness_ok else 0
                if score > best_score:
                    best_score = score; best = (r, g, b)
        return best
    except:
        return (70, 130, 180)

def color_is_dark(r, g, b):
    """True si el color es oscuro (para decidir texto blanco o negro)."""
    # Luminancia perceptual
    return (0.299*r + 0.587*g + 0.114*b) < 128


# ══ LRC Parser + Manager ════════════════════════════════════════════════
import re as _re, urllib.request, urllib.parse, json as _json

def parse_lrc(text):
    lines=[]; tag_re=_re.compile(r'\[(\d{2}):(\d{2})[.:]?(\d{0,3})\]')
    for raw in text.splitlines():
        raw=raw.strip(); tags=list(tag_re.finditer(raw))
        if not tags: continue
        lyric=raw[tags[-1].end():].strip()
        for m in tags:
            t=int(m.group(1))*60+int(m.group(2))+(int(m.group(3).ljust(2,"0")[:2]) if m.group(3) else 0)/100
            lines.append((t,lyric))
    lines.sort(key=lambda x:x[0]); return lines

class LyricsManager:
    def __init__(self): self._cache={}

    def get(self,song,callback):
        if song.path in self._cache:
            GLib.idle_add(callback,self._cache[song.path]); return
        threading.Thread(target=self._fetch,args=(song,callback),daemon=True).start()

    def _fetch(self,song,callback):
        lines=self._try_local(song) or self._try_online(song) or []
        self._cache[song.path]=lines; GLib.idle_add(callback,lines)

    def _try_local(self,song):
        lrc_path=Path(song.path).with_suffix(".lrc")
        if lrc_path.exists():
            try:
                t=lrc_path.read_text(encoding="utf-8",errors="replace")
                l=parse_lrc(t)
                if l: return l
            except: pass
        return None

    def _try_online(self,song):
        ...
        for timeout in (12,20):
            try:
                req=urllib.request.Request(f"https://lrclib.net/api/get?{params}",headers=hdrs)
                ...
                if plain: return [(i*3.0,ln) for i,ln in enumerate(plain.splitlines()) if ln.strip()]
                return None
            except urllib.error.HTTPError as e:
                print(f"[LRC] HTTP {e.code}: {song.title}"); return None
            except Exception as e:
                print(f"[LRC] Reintentando ({timeout}s): {e}")
        return None

    def current_index(self,lines,pos):
        idx=0
...
class FolderRow(Gtk.ListBoxRow):
    """ListBoxRow con path de carpeta embebido."""
...
class ArgOSMusicPlayer(Adw.ApplicationWindow):
    def __init__(self,app):
        ...
        self._ss=None
        self._lrc_lines=[]; self._lrc_index=-1; self._show_lyrics=False; self._lrc_token=0
        self._lyrics_mgr=LyricsManager()
        # CSS dinámico para color de portada
    ...
    def _apply_css(self):
        ...
        .lrc-line { font-size: 13px; padding: 3px 0; }
        .lrc-active { font-size: 15px; font-weight: bold; color: @accent_color; }
        .lrc-inactive { opacity: 0.45; }
        .lyrics-box { border-radius: 12px; background-color: alpha(@card_bg_color, 0.6); }
        """)
    ...
    def _build_player(self):
        ...
        # Cover + Lyrics
        cover_outer=Gtk.Box(orientation=Gtk.Orientation.VERTICAL,spacing=4)
        cover_outer.set_halign(Gtk.Align.CENTER)
        self._cstack=Gtk.Stack(); self._cstack.set_size_request(190,190)
        self._cstack.set_transition_type(Gtk.StackTransitionType.CROSSFADE)
        self._cstack.set_transition_duration(250)
        ...
        self._cstack.add_named(self._cimg,"cover")
        lf=Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        lf.add_css_class("lyrics-box"); lf.set_size_request(190,190)
        lsw=Gtk.ScrolledWindow()
        lsw.set_policy(Gtk.PolicyType.NEVER,Gtk.PolicyType.AUTOMATIC)
        lsw.set_size_request(190,190); lsw.set_vexpand(True)
        self._lyrics_box=Gtk.Box(orientation=Gtk.Orientation.VERTICAL,spacing=2)
        self._lyrics_box.set_margin_top(8); self._lyrics_box.set_margin_bottom(8)
        self._lyrics_box.set_margin_start(10); self._lyrics_box.set_margin_end(10)
        self._lyrics_vadj=lsw.get_vadjustment()
        lsw.set_child(self._lyrics_box)
        self._lrc_ph=Gtk.Label(label="Sin letras")
        self._lrc_ph.add_css_class("dim-label"); self._lrc_ph.add_css_class("caption")
        self._lrc_ph.set_valign(Gtk.Align.CENTER); self._lrc_ph.set_vexpand(True)
        lf.append(self._lrc_ph); lf.append(lsw)
        self._cstack.add_named(lf,"lyrics")
        self._lrc_btn=Gtk.Button(icon_name="format-justify-left-symbolic",tooltip_text="Mostrar letras")
        self._lrc_btn.add_css_class("flat"); self._lrc_btn.set_halign(Gtk.Align.CENTER)
        self._lrc_btn.connect("clicked",self._on_toggle_lyrics)
        cover_outer.append(self._cstack); cover_outer.append(self._lrc_btn)
        inner.append(cover_outer)
    ...
    def _build_library(self):
        ...
        # Vista carpetas
        self._folder_box=Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        self._vstack.add_titled_with_icon(self._wrap(self._folder_box),"folders","Carpetas","folder-music-symbolic")

        bar=Adw.ViewSwitcherBar(); bar.set_stack(self._vstack); bar.set_reveal(True)
        # Listener para cargar carpetas cuando se cambia a esa vista
        self._vstack.connect("notify::visible-child-name",self._on_view_changed)
    ...
    def _build_menu(self):
        ...
        s1=Gio.Menu()
        s1.append("Importar lista M3U","app.import_m3u")
        s1.append("Exportar lista M3U","app.export_m3u")
        m.append_section("Listas de reproducción",s1)
        m.append("Mini reproductor","app.mini_player")
    ...
    def _on_queue_btn(self,btn,idx):
        if btn.get_active():
            ...
            self.mqm.switch(idx)
            if self._ss is None: return  # UI aún no inicializada
    ...
    def _play_current(self):
        ...
        if n: self._ss.items_changed(0,n,n)
        self.mpris.notify()
        # Notificar mini player si está abierto
        if hasattr(self,'_mini_win') and self._mini_win:
            GLib.idle_add(self._mini_win.update_song,s)
        # Resetear letras para la nueva canción
        self._lrc_lines=[]; self._lrc_index=-1
        self._lrc_token=id(s)  # invalida respuestas en vuelo
        if self._show_lyrics: self._load_lyrics(s)
    ...
    def _load_cover(self, s):
        if s.cover:
            try:
                ...
                pixbuf=ld.get_pixbuf()
                # Usar Gdk.Texture en lugar del deprecado set_pixbuf
                try:
                    texture=Gdk.Texture.new_for_pixbuf(pixbuf)
                    self._cimg.set_paintable(texture)
                except Exception:
                    self._cimg.set_pixbuf(pixbuf)
                self._cstack.set_visible_child_name("cover")
                def _color_thread(pb):
                    color=dominant_color(pb)
                    GLib.idle_add(self._apply_cover_color_rgb, *color)
                threading.Thread(target=_color_thread,args=(pixbuf,),daemon=True).start()
                return
            except: pass
        self._cstack.set_visible_child_name("ph")
        self._reset_cover_color()

    def _apply_cover_color_rgb(self, r, g, b):
        """Llamado desde idle_add con el color ya calculado en hilo."""
        is_dark = Adw.StyleManager.get_default().get_dark()
        fg = "white" if color_is_dark(r,g,b) else "#111111"
        # Variante más clara en dark, más oscura en light para contraste
        if is_dark:
            r2,g2,b2 = min(255,int(r*1.3)), min(255,int(g*1.3)), min(255,int(b*1.3))
        else:
            r2,g2,b2 = int(r*0.78), int(g*0.78), int(b*0.78)
        # Sin @define-color: GTK4 los acepta pero los warnings vienen
        # de bloques vacíos; usamos solo selectores concretos
        # Cada regla en línea separada, sin punto y coma final de bloque
        css_rules = [
            f".active-row {{ background-color: rgba({r},{g},{b},0.18); border-radius: 8px; }}",
            f".cover-ph {{ background-color: rgba({r},{g},{b},0.10); }}",
            f"scale trough highlight {{ background-color: rgb({r},{g},{b}); }}",
            f"button.suggested-action {{ background-color: rgb({r2},{g2},{b2}); color: {fg}; }}",
            f"button.suggested-action:hover {{ background-color: rgb({min(255,r2+20)},{min(255,g2+20)},{min(255,b2+20)}); color: {fg}; }}",
        ]
        self._dynamic_css.load_from_data(("\n".join(css_rules)).encode())

    def _reset_cover_color(self):
        self._dynamic_css.load_from_data(b"")
    ...
    def _update_pos(self,pos,dur):
        ...
        if self._show_lyrics and self._lrc_lines: self._sync_lyrics(pos)
    ...
    # ── Letras LRC ─────────────────────────────
    def _on_toggle_lyrics(self,_):
        self._show_lyrics=not self._show_lyrics
        if self._show_lyrics:
            self._cstack.set_visible_child_name("lyrics")
            self._lrc_btn.set_icon_name("image-x-generic-symbolic")
            self._lrc_btn.set_tooltip_text("Mostrar portada")
            if not self._lrc_lines:
                s=self.mqm.current
                if s: self._load_lyrics(s)
        else:
            s=self.mqm.current
            self._cstack.set_visible_child_name("cover" if (s and s.cover) else "ph")
            self._lrc_btn.set_icon_name("format-justify-left-symbolic")
            self._lrc_btn.set_tooltip_text("Mostrar letras")

    def _load_lyrics(self,song):
        # Token: ignorar respuestas de canciones anteriores
        self._lrc_token=id(song)
        token=self._lrc_token
        self._lrc_lines=[]; self._lrc_index=-1
        self._lrc_ph.set_label("Buscando letras...")
        self._lrc_ph.set_visible(True)
        c=self._lyrics_box.get_first_child()
        while c: n=c.get_next_sibling(); self._lyrics_box.remove(c); c=n
        def on_loaded(lines):
            if self._lrc_token!=token: return  # respuesta de canción anterior
            self._lrc_lines=lines; self._lrc_index=-1
            if not lines:
                self._lrc_ph.set_label("Sin letras disponibles")
                self._lrc_ph.set_visible(True); return
            self._lrc_ph.set_visible(False)
            for _t,txt in lines:
                lbl=Gtk.Label(label=txt or "·")
                lbl.add_css_class("lrc-line"); lbl.add_css_class("lrc-inactive")
                lbl.set_wrap(True); lbl.set_xalign(0.5)
                self._lyrics_box.append(lbl)
            self._sync_lyrics(self.engine.position)
        self._lyrics_mgr.get(song,on_loaded)

    def _sync_lyrics(self,pos):
        if not self._lrc_lines: return
        new_idx=self._lyrics_mgr.current_index(self._lrc_lines,pos)
        if new_idx==self._lrc_index: return
        self._lrc_index=new_idx
        labels=[]; c=self._lyrics_box.get_first_child()
        while c: labels.append(c); c=c.get_next_sibling()
        for i,lbl in enumerate(labels):
            if i==new_idx:
                lbl.remove_css_class("lrc-inactive"); lbl.add_css_class("lrc-active")
            else:
                lbl.remove_css_class("lrc-active"); lbl.add_css_class("lrc-inactive")
        if 0<=new_idx<len(labels):
            GLib.idle_add(self._scroll_to_lrc,labels[new_idx])

    def _scroll_to_lrc(self,lbl):
        try:
            adj=self._lyrics_vadj
            # GTK4: computar posición del widget relativa al box padre
            ok,x,y=lbl.translate_coordinates(self._lyrics_box,0,0)
            if ok:
                h=lbl.get_height() or 20
                center=y+h/2; page=adj.get_page_size()
                adj.set_value(max(0,min(center-page/2,adj.get_upper()-page)))
        except: pass
        return False

    def _on_toggle_theme(self,_):
        mgr=Adw.StyleManager.get_default()
        current=mgr.get_color_scheme()
        if current==Adw.ColorScheme.FORCE_DARK:
            mgr.set_color_scheme(Adw.ColorScheme.FORCE_LIGHT)
        else:
            mgr.set_color_scheme(Adw.ColorScheme.FORCE_DARK)
        self._update_theme_btn()
        # Reaplicar color de portada para ajustar al nuevo tema
        s=self.mqm.current
        if s and s.cover:
            try:
                ld=GdkPixbuf.PixbufLoader(); ld.write(s.cover); ld.close()
                color = dominant_color(ld.get_pixbuf())
                self._apply_cover_color_rgb(*color)
            except: pass

    def _update_theme_btn(self):
        mgr=Adw.StyleManager.get_default()
        is_dark=mgr.get_dark()
        icon="weather-clear-symbolic" if is_dark else "weather-clear-night-symbolic"
        tip="Cambiar a modo claro" if is_dark else "Cambiar a modo oscuro"
        self._theme_btn.set_icon_name(icon)
        self._theme_btn.set_tooltip_text(tip)
    # ── Vista de carpetas ───────────────────────
    def _on_view_changed(self,stack,_):
        if stack.get_visible_child_name()=="folders":
            self._build_folder_tree()

    def _build_folder_tree(self):
        # Limpiar
        c=self._folder_box.get_first_child()
        while c: n=c.get_next_sibling(); self._folder_box.remove(c); c=n

        # Obtener carpetas únicas de la DB
        rows=self.db.conn.execute(
            "SELECT DISTINCT folder FROM songs ORDER BY folder").fetchall()
        folders=[r[0] for r in rows if r[0]]

        if not folders:
            lbl=Gtk.Label(label="Abre una carpeta de música primero")
            lbl.add_css_class("dim-label"); lbl.set_margin_top(32)
            self._folder_box.append(lbl); return
        ...
        # Construir ListBox con filas
        lb=Gtk.ListBox(); lb.set_selection_mode(Gtk.SelectionMode.NONE)
        lb.add_css_class("navigation-sidebar")
        # Conectar row-activated en el ListBox (más confiable que en cada row)
        lb.connect("row-activated", lambda lb, row: self._on_folder_row_act(row))
        self._add_folder_rows(lb, root_map, 0, folders)
        self._folder_box.append(lb)

    def _add_folder_rows(self,lb,node,depth,all_folders):
        for key,child in sorted(node.items()):
            if key=="__path__": continue
            path=child.get("__path__")
            # FolderRow guarda el path como atributo Python normal
            row=FolderRow(folder_path=path)
            hbox=Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL,spacing=8)
            hbox.set_margin_start(12+depth*16)
            hbox.set_margin_end(12); hbox.set_margin_top(6); hbox.set_margin_bottom(6)
            icon=Gtk.Image.new_from_icon_name(
                "folder-music-symbolic" if path else "folder-symbolic")
            icon.set_pixel_size(20); icon.set_opacity(0.7)
            name_lbl=Gtk.Label(label=key)
            name_lbl.set_halign(Gtk.Align.START); name_lbl.set_hexpand(True)
            name_lbl.add_css_class("lib-title")
            hbox.append(icon); hbox.append(name_lbl)
            if path:
                cnt=self.db.conn.execute(
                    "SELECT COUNT(*) FROM songs WHERE folder=?",(path,)).fetchone()[0]
                sub=Gtk.Label(label=f"{cnt} canciones")
            ...
            subkeys={k:v for k,v in child.items() if k!="__path__"}
            if subkeys:
                self._add_folder_rows(lb,child,depth+1,all_folders)

    def _on_folder_row_act(self,row):
        if not isinstance(row,FolderRow) or not row.folder_path: return
        songs=self.db.songs_in_folder(row.folder_path)
        if songs:
            self._subview(f"📁 {Path(row.folder_path).name}",songs)
        else:
            self._lstat.set_label(f"Sin canciones en {Path(row.folder_path).name}")

    # ── Listas M3U ──────────────────────────────
    def _export_m3u(self,songs,path):
        try:
            lines=["#EXTM3U"]
            for s in songs:
                lines.append(f"#EXTINF:{s.duration},{s.artist} - {s.title}")
                lines.append(s.path)
    ...
    def _import_m3u(self,m3u_path):
        ...
        try:
            ...
            for line in lines:
                ...
                # Normalizar separadores de ruta (compatibilidad Windows)
                line=line.replace("\\","/")
                # Ruta absoluta o relativa al .m3u
                p=Path(line)
                ...
                if p.exists() and sp not in seen:
                    try:
                        s=Song.from_path(sp); songs.append(s); seen.add(sp)
                    except: pass
        except Exception as e:
            print(f"[M3U] Error: {e}")
        return songs

    def _on_export_m3u(self,_=None):
        songs=self.mqm.songs
        if not songs:
            self._lstat.set_label("No hay canciones en la cola"); return
        d=Gtk.FileDialog(); d.set_title("Exportar lista M3U")
        ff=Gtk.FileFilter(); ff.set_name("Listas M3U (*.m3u)")
        ff.add_pattern("*.m3u"); ff.add_pattern("*.M3U")
        fls=Gio.ListStore.new(Gtk.FileFilter); fls.append(ff); d.set_filters(fls)
        d.set_initial_name("lista.m3u")
        # Pasar self (la ventana principal) como parent
        d.save(self,None,self._m3u_save_cb)

    def _m3u_save_cb(self,dialog,result):
        try:
            f=dialog.save_finish(result)
            if f:
                ok=self._export_m3u(self.mqm.songs,f.get_path())
                self._lstat.set_label("Lista exportada ✓" if ok else "Error al exportar")
        except GLib.Error: pass

    def _on_import_m3u(self,_=None):
        d=Gtk.FileDialog(); d.set_title("Importar lista M3U")
        ff=Gtk.FileFilter(); ff.set_name("Listas M3U (*.m3u)")
        ff.add_pattern("*.m3u"); ff.add_pattern("*.M3U")
        fls=Gio.ListStore.new(Gtk.FileFilter); fls.append(ff); d.set_filters(fls)
        d.open(self,None,self._m3u_open_cb)

    def _m3u_open_cb(self,dialog,result):
        try:
            f=dialog.open_finish(result)
            if f:
                songs=self._import_m3u(f.get_path())
                if songs:
                    self._play_list(songs,0)
                    self._lstat.set_label(f"M3U: {len(songs)} canciones")
                else:
                    self._lstat.set_label("No se encontraron archivos en la lista")
        except GLib.Error: pass

    # ── Mini reproductor flotante ────────────────
    def _on_mini_player(self,_=None):
        if hasattr(self,"_mini_win") and self._mini_win:
            self._mini_win.present(); return
        self._mini_win=MiniPlayer(self)
        self._mini_win.present()


# ══ Mini Reproductor Flotante ═══════════════════════════════════════════
class MiniPlayer(Gtk.Window):
    """Ventana compacta flotante con portada, título y controles."""
    def __init__(self, main_win):
        ...
        # Usamos always-on-top vía la acción del gestor de ventanas
        self.connect('realize', self._set_keep_above_wm)
        ...
        p=Gtk.CssProvider()
        p.load_from_data(b"""
        .mini-cover { border-radius: 8px; }
        .mini-title { font-size: 13px; font-weight: bold; }
        .mini-artist { font-size: 11px; }
        .mini-bg { background-color: alpha(@card_bg_color, 0.95); }
        """)
        Gtk.StyleContext.add_provider_for_display(
        ...
        root=Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        root.add_css_class("mini-bg")

        # Seek bar fina arriba
        self._seek=Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL,0,1,0.01)
        self._seek.set_draw_value(False); self._seek.set_size_request(-1,4)
        self._seek.add_css_class("flat")
        sg=Gtk.GestureClick.new()
        sg.connect("released",self._on_seek); self._seek.add_controller(sg)
        root.append(self._seek)

        # Fila principal
        main_row=Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL,spacing=10)
        main_row.set_margin_start(10); main_row.set_margin_end(10)
        main_row.set_margin_top(8); main_row.set_margin_bottom(8)

        # Portada
        self._cover_stack=Gtk.Stack(); self._cover_stack.set_size_request(64,64)
        ph=Gtk.Box(); ph.add_css_class("cover-ph"); ph.set_size_request(64,64)
        ic=Gtk.Image.new_from_icon_name("audio-x-generic-symbolic")
        ic.set_pixel_size(28); ic.set_opacity(0.3); ic.set_hexpand(True); ic.set_valign(Gtk.Align.CENTER)
        ph.append(ic); self._cover_stack.add_named(ph,"ph")
        self._cover=Gtk.Picture(); self._cover.set_size_request(64,64)
        self._cover.add_css_class("mini-cover"); self._cover.set_content_fit(Gtk.ContentFit.COVER)
        self._cover_stack.add_named(self._cover,"cover")
        main_row.append(self._cover_stack)

        # Info + controles
        info_ctrl=Gtk.Box(orientation=Gtk.Orientation.VERTICAL,spacing=4)
        info_ctrl.set_hexpand(True); info_ctrl.set_valign(Gtk.Align.CENTER)

        self._lbl_t=Gtk.Label(label="Sin reproducción")
        self._lbl_t.add_css_class("mini-title")
        self._lbl_t.set_halign(Gtk.Align.START); self._lbl_t.set_ellipsize(Pango.EllipsizeMode.END)
        self._lbl_a=Gtk.Label(label="—")
        self._lbl_a.add_css_class("mini-artist"); self._lbl_a.add_css_class("dim-label")
        self._lbl_a.set_halign(Gtk.Align.START); self._lbl_a.set_ellipsize(Pango.EllipsizeMode.END)
        info_ctrl.append(self._lbl_t); info_ctrl.append(self._lbl_a)

        # Controles
        ctrl=Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL,spacing=0)
        btn_prev=Gtk.Button(icon_name="media-skip-backward-symbolic")
        btn_prev.add_css_class("flat"); btn_prev.connect("clicked",lambda _: main_win._on_prev())
        self._btn_play=Gtk.Button(icon_name="media-playback-start-symbolic")
        self._btn_play.add_css_class("flat")
        self._btn_play.connect("clicked",lambda _: main_win._on_play_pause())
        btn_next=Gtk.Button(icon_name="media-skip-forward-symbolic")
        btn_next.add_css_class("flat"); btn_next.connect("clicked",lambda _: main_win._on_next())
        btn_expand=Gtk.Button(icon_name="view-restore-symbolic",tooltip_text="Abrir reproductor completo")
        btn_expand.add_css_class("flat"); btn_expand.connect("clicked",self._on_expand)
        for b in (btn_prev,self._btn_play,btn_next,btn_expand): ctrl.append(b)
        info_ctrl.append(ctrl)
        main_row.append(info_ctrl)
    ...
    def _sync_from_main(self):
        s=self.main.mqm.current
        if s:
            self._lbl_t.set_label(s.display_title)
            self._lbl_a.set_label(s.artist)
            self._seek.set_range(0,max(s.duration,1))
            # Portada
            if s.cover:
                try:
                    ld=GdkPixbuf.PixbufLoader(); ld.write(s.cover); ld.close()
                    try:
                        tex=Gdk.Texture.new_for_pixbuf(ld.get_pixbuf())
                        self._cover.set_paintable(tex)
                    except:
                        self._cover.set_pixbuf(ld.get_pixbuf())
                    self._cover_stack.set_visible_child_name("cover")
                except: self._cover_stack.set_visible_child_name("ph")
            else:
                self._cover_stack.set_visible_child_name("ph")
        if self.main.engine.is_playing:
    ...
    def _tick(self):
        ...
        pos=self.main.engine.position; dur=self.main.engine.duration
        if dur>0: self._seek.set_value(pos)
    ...
    def _on_seek(self,gesture,n,x,y):
        val=self._seek.get_value(); self.main.engine.seek(val)

    def _on_expand(self,_):
        self.main.present(); self.close()

    def _set_keep_above_wm(self,_):
        """Intenta mantener la ventana siempre encima via GDK."""
        try:
            surface=self.get_surface()
            if hasattr(surface,'inhibit_system_shortcuts'): pass
            # Gtk4 / Wayland: usar xdg-toplevel si está disponible
            if hasattr(surface,'set_keep_above'):
                surface.set_keep_above(True)
        except: pass

    def _on_close(self,_):
        self.main._mini_win=None; return False

    def update_song(self,s):
        """Llamado desde el reproductor principal al cambiar canción."""
        self._lbl_t.set_label(s.display_title)
        self._lbl_a.set_label(s.artist)
        self._seek.set_range(0,max(s.duration,1))
        if s.cover:
            try:
                ...
                try:
                    ...
                    self._cover.set_pixbuf(ld.get_pixbuf())
                self._cover_stack.set_visible_child_name("cover"); return
            except: pass
        self._cover_stack.set_visible_child_name("ph")
...
class ArgOSMusicApp(Adw.Application):
    def __init__(self):
        ...
        for name,cb in [
            ...
            ("import_m3u",  lambda a,p: self.win._on_import_m3u()),
            ("export_m3u",  lambda a,p: self.win._on_export_m3u()),
            ("mini_player", lambda a,p: self.win._on_mini_player()),
            ("about",       self._about),
        ]:
